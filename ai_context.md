...
- **divgroups:** Includes `gender` (`boys` / `girls`) for boys vs girls groups; default `boys` for existing rows. Migration: `2025_03_16_120000_add_gender_to_divgroups_table`.
...
## 8. Division/Group gender (phased)

- **Phase 1 (done):** `divgroups.gender` added (`boys` / `girls`). Create/Edit group forms and division show table include Boys/Girls. Existing rows default to `boys`.
- **Phase 2 (done):** Registration and group assignment filter by wrestler gender. `wrestlers.wr_gender` (Boy/Girl) maps to group `gender` (boys/girls). Registration only assigns to a group matching the wrestler’s gender; Move Wrestler form only lists same-gender groups and rejects moving to the wrong gender.
- **Phase 3 (later):** Reporting and projection views: optional filter by boys/girls groups; team sheet or scan sheet by gender if required.

---

## 9. Features NOT yet developed (implement when needed)
