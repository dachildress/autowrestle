...
<form method="post" action="{{ route('manage.groups.store', [$tournament->id, $division->id]) }}">
    @csrf
    <p>
        <label for="Name">Name</label>
        <input type="text" name="Name" id="Name" value="{{ old('Name') }}" maxlength="25" required>
        @error('Name') <span class="error">{{ $message }}</span> @enderror
    </p>
    <p>
        <label for="MinAge">Minimum Age</label>
        <input type="number" name="MinAge" id="MinAge" value="{{ old('MinAge') }}" min="3" max="19" required>
        @error('MinAge') <span class="error">{{ $message }}</span> @enderror
    </p>
    <p>
        <label for="MaxAge">Max Age</label>
        <input type="number" name="MaxAge" id="MaxAge" value="{{ old('MaxAge') }}" min="3" max="19" required>
        @error('MaxAge') <span class="error">{{ $message }}</span> @enderror
    </p>
    <p>
        <label for="MinGrade">Minimum Grade (-1 = Pre-K, 0 = K)</label>
        <input type="number" name="MinGrade" id="MinGrade" value="{{ old('MinGrade') }}" min="-1" max="12" required>
        @error('MinGrade') <span class="error">{{ $message }}</span> @enderror
    </p>
    <p>
        <label for="MaxGrade">Max Grade</label>
        <input type="number" name="MaxGrade" id="MaxGrade" value="{{ old('MaxGrade') }}" min="-1" max="12" required>
        @error('MaxGrade') <span class="error">{{ $message }}</span> @enderror
    </p>
    <p>
        <label for="MaxWeightDiff">Max Weight Diff</label>
        <input type="number" name="MaxWeightDiff" id="MaxWeightDiff" value="{{ old('MaxWeightDiff', 10) }}" min="0" max="20" required>
        @error('MaxWeightDiff') <span class="error">{{ $message }}</span> @enderror
    </p>
    <p>
        <label for="MaxPwrDiff">Max Power Diff</label>
        <input type="number" name="MaxPwrDiff" id="MaxPwrDiff" value="{{ old('MaxPwrDiff', 10) }}" min="0" max="30" required>
        @error('MaxPwrDiff') <span class="error">{{ $message }}</span> @enderror
    </p>
    <p>
        <label for="MaxExpDiff">Max Experience Diff</label>
        <input type="number" name="MaxExpDiff" id="MaxExpDiff" value="{{ old('MaxExpDiff', 10) }}" min="0" max="30" required>
        @error('MaxExpDiff') <span class="error">{{ $message }}</span> @enderror
    </p>
    <p>
        <label for="BracketType">Bracket Type</label>
        <input type="text" name="BracketType" id="BracketType" value="{{ old('BracketType', 'Round Robin') }}" maxlength="20">
