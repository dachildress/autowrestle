...
<div class="container main">
    <x-card :padding="true" class="max-w-2xl">
        <h1 class="text-xl font-bold text-aw-primary mb-2">Edit Group</h1>
        ...
        <form method="post" action="{{ route('manage.groups.update', [$tournament->id, $division->id, $group->id]) }}" class="space-y-5">
            @csrf

            <div>
                <label for="Name" class="block text-sm font-medium text-slate-700">Group Name <span class="text-red-600">*</span></label>
                <input type="text" name="Name" id="Name" value="{{ old('Name', $group->Name) }}" maxlength="25" required class="mt-1 block w-full rounded-md border border-slate-300 py-2 px-3 text-sm">
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-4">
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="gender" value="boys" {{ old('gender', $group->gender ?? 'boys') === 'boys' ? 'checked' : '' }} class="rounded-full border border-slate-300 text-aw-primary">
                        <span class="text-sm text-slate-700">Boys</span>
                    </label>
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="gender" value="girls" {{ old('gender', $group->gender ?? 'boys') === 'girls' ? 'checked' : '' }} class="rounded-full border border-slate-300 text-aw-primary">
                        <span class="text-sm text-slate-700">Girls</span>
                    </label>
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="gender" value="coed" {{ old('gender', $group->gender ?? 'boys') === 'coed' ? 'checked' : '' }} class="rounded-full border border-slate-300 text-aw-primary">
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <input type="number" name="MinAge" id="MinAge" value="{{ old('MinAge', $group->MinAge) }}" min="3" max="19" required class="mt-1 block w-full rounded-md border border-slate-300 py-2 px-3 text-sm">
                ...
                <div>
                    ...
                    <input type="number" name="MaxAge" id="MaxAge" value="{{ old('MaxAge', $group->MaxAge) }}" min="3" max="19" required class="mt-1 block w-full rounded-md border border-slate-300 py-2 px-3 text-sm">
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <select name="MinGrade" id="MinGrade" required class="mt-1 block w-full rounded-md border border-slate-300 py-2 px-3 text-sm">
                        @foreach([-1 => 'Pre-K (-1)', 0 => 'K (0)'] + array_combine(range(1, 12), range(1, 12)) as $val => $label)
                            <option value="{{ $val }}" {{ old('MinGrade', (string)$group->MinGrade) === (string)$val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="MaxGrade" id="MaxGrade" required class="mt-1 block w-full rounded-md border border-slate-300 py-2 px-3 text-sm">
                        @foreach([-1 => 'Pre-K (-1)', 0 => 'K (0)'] + array_combine(range(1, 12), range(1, 12)) as $val => $label)
                            <option value="{{ $val }}" {{ old('MaxGrade', (string)$group->MaxGrade) === (string)$val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <input type="number" name="MaxWeightDiff" id="MaxWeightDiff" value="{{ old('MaxWeightDiff', $group->MaxWeightDiff) }}" min="0" max="20" required class="mt-1 block w-full rounded-md border border-slate-300 py-2 px-3 text-sm">
                ...
                <div>
                    ...
                    <input type="number" name="MaxPwrDiff" id="MaxPwrDiff" value="{{ old('MaxPwrDiff', $group->MaxPwrDiff) }}" min="0" max="30" required class="mt-1 block w-full rounded-md border border-slate-300 py-2 px-3 text-sm">
            ...
            <div>
                ...
                <select name="MaxExpDiff" id="MaxExpDiff" required class="mt-1 block w-full max-w-xs rounded-md border border-slate-300 py-2 px-3 text-sm">
                    @foreach(range(0, 30) as $n)
                        <option value="{{ $n }}" {{ old('MaxExpDiff', (string)$group->MaxExpDiff) === (string)$n ? 'selected' : '' }}>{{ $n }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                @php $bracketOptions = ['Round Robin' => 'Round Robin', 4 => '4', 5 => '5', 6 => '6', 8 => '8']; $currentBt = old('BracketType', $group->BracketType ?? 'Round Robin'); @endphp
                <select name="BracketType" id="BracketType" class="mt-1 block w-full max-w-xs rounded-md border border-slate-300 py-2 px-3 text-sm">
                    @foreach($bracketOptions as $val => $label)
                        <option value="{{ $val }}" {{ (string)$currentBt === (string)$val ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                    @if(!array_key_exists($currentBt, $bracketOptions))
                        <option value="{{ e($currentBt) }}" selected>{{ e($currentBt) }}</option>
                    @endif
                </select>
