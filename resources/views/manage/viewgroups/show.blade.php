...
@section('title', 'View Groups – ' . $group->display_name . ' – ' . $tournament->TournamentName)
...
@else
    <table class="table-dark">
        ...
        <tbody>
            ...
            @php
                $wrestlerGender = $w->wrestler?->wr_gender ?? null;
                $groupGender = $group->gender ?? 'boys';
                $genderMatches = $groupGender === 'coed'
                    || ($wrestlerGender === 'Boy' && $groupGender === 'boys')
                    || ($wrestlerGender === 'Girl' && $groupGender === 'girls');
            @endphp
            <tr @if(!$genderMatches && $wrestlerGender) class="warning" style="background-color: #fff3cd;" @endif>
                <td>
                    {{ $w->wr_first_name }} {{ $w->wr_last_name }}
                    @if(!$genderMatches && $wrestlerGender)
                        <span class="error" title="Wrestler gender ({{ $wrestlerGender }}) does not match this {{ $groupGender }} group."> ⚠ Wrong group</span>
                    @endif
                </td>
                <td>{{ $w->wr_club }}</td>
