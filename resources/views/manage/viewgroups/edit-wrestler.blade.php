...
<form method="post" action="{{ route('manage.viewgroups.updateWrestler', [$tournament->id, $tw->id]) }}" class="form-horizontal">
    ...
    @if(isset($allowedGroups) && $allowedGroups->isNotEmpty())
    <div class="form-group">
        <label for="group_id">Group</label>
        <select name="group_id" id="group_id" style="padding: 6px 10px; min-width: 220px;">
            <option value="">— Keep current group —</option>
            @foreach($allowedGroups as $g)
                <option value="{{ $g->id }}" {{ old('group_id', $gid) == $g->id ? 'selected' : '' }}>{{ $g->division_name }}</option>
            @endforeach
        </select>
        <span class="text-muted" style="font-size: 0.9rem;">Only groups matching this wrestler's gender are shown.</span>
    </div>
    @endif
